...
class KML extends GeoAdapter
{
        ...
        if ($placemark_elements->length) {
            foreach ($placemark_elements as $placemark) {
                foreach ($placemark->childNodes as $child) {
                    ...
                    $node_name = 'multigeometry' == $child->nodeName ? 'geometrycollection' : $child->nodeName;
            ...
            $node_name = 'multigeometry' == $this->xmlobj->documentElement->nodeName ? 'geometrycollection' : $this->xmlobj->documentElement->nodeName;
            if (array_key_exists($node_name, $geom_types)) {
                $function = 'parse'.$geom_types[$node_name];
    ...
    protected function parseGeometryCollection($xml)
    {
        ...
        $geom_types = GeoPHP::geometryList();
        foreach ($xml->childNodes as $child) {
            $nodeName = ('linearring' == $child->nodeName) ? 'linestring' : $child->nodeName;
